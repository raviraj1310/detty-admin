                ...
                <>
                    ...
                    >
                        <div className='py-1'>
                            ...
                            <button onClick={() => { if (typeof openCustomer === 'function') openCustomer(row); setIsOpen(false); }} className='flex items-center w-full px-4 py-2 text-sm text-gray-700 hover:bg-gray-100'>
                                <span className='mr-3 text-gray-500'>
                            ...
                            {(() => {
                                const isDownloading = String(downloadingId || '') === String(row.orderId || row.id)
                                return (
                                    <button onClick={() => { if (typeof downloadReceipt === 'function') downloadReceipt(row); setIsOpen(false); }} disabled={isDownloading} className={`flex items-center w-full px-4 py-2 text-sm ${isDownloading ? 'text-[#8C93AF] cursor-not-allowed opacity-70' : 'text-gray-700 hover:bg-gray-100'}`}>
                                        <span className='mr-3 text-gray-500'>
...
export default function MerchandisePage() {
        ...
        orders.forEach(o => {
            ...
            items.forEach((it, idx) => {
                ...
                rows.push({
                    ...
                    category,
                    quantity: qty ? `${qty} x ${toCurrency(unitPrice)}` : `${toCurrency(unitPrice)}`,
    ...
    const filteredMerchandise = merchandiseData.filter(merchandise => {
        ...
        const dateDigits = String(merchandise.eventDate || '').replace(/[^0-9]/g, '');
        const matchesText = name.includes(term) || category.includes(term) || dateStr.includes(term);
    ...
    const openCustomer = (o) => {
        const merchandise = merchandiseData.find(item => String(item.id) === String(o?.id)) || o
        setSelectedOrder(merchandise)
        setShowOrderDetails(true)
    }
                    ...
                    <table className="w-full">
                        ...
                        <tbody className="bg-white divide-y divide-gray-200">
                            {filteredMerchandise.map((merchandise) => (
                                <tr key={merchandise.id} className="hover:bg-gray-50 border-b border-gray-100">
                                    ...
                                    <td className="px-3 py-3 whitespace-nowrap">
                                        <span className="text-sm font-medium text-gray-900">{merchandise.category}</span>
                                    </td>
                                    <td className="px-3 py-3 whitespace-nowrap text-sm text-gray-900">
                                        <div>{merchandise.quantity}</div>
                                    </td>
                                    <td className="px-3 py-3 whitespace-nowrap">
                                        <span className="text-sm font-semibold text-gray-900">{merchandise.totalAmount}</span>
                                    </td>
                                    <td className="px-3 py-3 whitespace-nowrap">
                                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${getActivityStatusColor(merchandise.activityStatus)}`}>
                                            • {merchandise.activityStatus}
                                        </span>
                                    </td>
                                    <td className="px-3 py-3 whitespace-nowrap">
                                        <span className={`inline-flex items-center px-3 py-1 rounded-full text-xs font-medium ${getPaymentStatusColor(merchandise.paymentStatus)}`}>
                                            {merchandise.paymentStatus}
                                        </span>
                                    </td>
                                    <td className="px-6 py-3 whitespace-nowrap text-right relative">
                                        <ActionDropdown row={merchandise} downloadingId={downloadingId} openCustomer={openCustomer} downloadReceipt={downloadReceipt} />
                                    </td>
                                </tr>
                            ))}
                        </tbody>
                ...
                <div className="fixed inset-0 bg-black/40 bg-opacity-30 flex items-center justify-center z-50">
                    <div className="bg-white rounded-xl shadow-2xl w-[420px] max-w-md mx-4 border border-gray-100">
                        ...
                        <div className="px-6 py-5">
                            {/* Order ID */}
                            <div className="mb-5">
                                <h4 className="text-base font-semibold text-gray-900 mb-3">Order ID 102832</h4>
                                <div className="flex justify-between items-center bg-gray-50 p-3 rounded-lg">
                                    <span className="text-sm text-gray-700">₦2,000 x 2 DettyFusion Tshirts</span>
                                    <span className="font-semibold text-gray-900">₦4,000.00</span>
                                </div>
                            </div>

                            {/* Billing Details */}
                            <div className="mb-5">
                                <h4 className="text-base font-semibold text-gray-900 mb-4">Billing Details</h4>
                                
                                <div className="space-y-3">
                                    <div className="flex justify-between items-center py-1">
                                        <span className="text-sm text-gray-600">Buyer's Email Address</span>
                                        <span className="text-sm font-medium text-gray-900">loveshurter@gmail.com</span>
                                    </div>
                                    ...
                                    <div className="flex justify-between items-center py-1">
                                        ...
                                        <span className="text-sm font-medium text-gray-900">₦4,010.00</span>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="border-t border-gray-200 pt-4">
                                <div className="flex justify-between items-center">
                                    ...
                                    <span className="text-xl font-bold text-orange-500">₦4,010.00</span>
                                </div>
                            </div>
                        </div>
                    </div>
