...
import { getRecoveryBookingById } from '@/services/v2/other-recovery-services/otherRecoveryServices.service'
...
const mapRecoveryBookingFromApi = api => {
  const data = api?.data || {}

  const svc = data?.recoveryServiceId || {}
  const slot = data?.recoverySlotId || {}
  const session = data?.recoverySessionId || {}
  ...
  const paymentStatusLabel =
    payment === 'success' || payment === 'completed' ? 'Success' : 'Pending'
  ...
  return {
    id: data?._id,
    orderId: data?.transactionRef || data?.transactionId || data?._id,
    issuedOn: data?.createdAt,
    visitDate: data?.arrivalDate,
    serviceName: svc?.recoveryServiceName || 'Recovery Service',
    serviceLocation: svc?.location || '—',
    serviceImage:
      toUploadUrl(svc?.image, 'image') || '/images/dashboard/image-1.webp',
    openingHours:
      svc?.duration && svc?.startTime && svc?.endTime
        ? `${svc.duration}, ${svc.startTime} - ${svc.endTime}`
        : `${svc?.startTime || ''} - ${svc?.endTime || ''}` || '—',
    sessionName: session?.recoveryServiceSessionName || 'Session',
    sessionDuration: session?.duration || '—',
    sessionDetail: session?.detail || '',
    slotName: slot?.slotName || '',
    slotTime: slot?.slotTime || '',
    dateTime: formatEventDateTime(data?.arrivalDate, slot?.slotTime),
    passLabel: `${qty}× ${session?.recoveryServiceSessionName || 'Session'}`,
    passPrice:
    ...
    discountApplied: data?.pricing?.discountApplied ?? 0,
    total: data?.pricing?.total ?? data?.totalAmount ?? 0,
    finalPayableAmount:
      data?.finalPayableAmount ??
      data?.pricing?.total ??
      data?.totalAmount ??
      0,
    transactionRef: data?.transactionRef || data?.transactionId || '—',
...
export default function RecoveryServiceBookingView () {
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      setLoading(true)
      try {
        const res = await getRecoveryBookingById(id)
        if (!res?.success) {
          setBooking(null)
          return
        }
        setBooking(mapRecoveryBookingFromApi(res))
      } catch (err) {
        setBooking(null)
      } finally {
        setLoading(false)
  ...
  const qrValue = booking.orderId || booking.id || id || ''
      ...
      <div className='mx-auto max-w-3xl overflow-hidden rounded-3xl bg-white shadow-sm'>
        ...
        <div className='flex flex-col gap-6 border-b border-[#E2E8F0] px-6 py-6 md:flex-row md:items-start md:justify-between md:px-8 md:py-8'>
          <div className='flex flex-1 gap-4'>
            <div className='relative h-[70px] w-[100px] shrink-0 overflow-hidden rounded-lg bg-gray-100 md:h-20 md:w-28'>
              <Image
                src={booking.serviceImage || '/images/dashboard/image-1.webp'}
            ...
            <div className='min-w-0'>
              ...
              <p className='text-xs text-[#64748B] md:text-sm'>
                {booking.sessionName} • {booking.sessionDuration}
              </p>
              <p className='mt-1 text-sm text-[#64748B]'>{booking.dateTime}</p>
        ...
        <div className='border-b border-dashed border-[#E2E8F0] px-6 py-4 md:px-8'>
          ...
          <div className='mt-4 space-y-2 text-sm'>
            ...
            <div className='flex items-center justify-between'>
              <span className='text-[#64748B]'>Final Payable Amount</span>
