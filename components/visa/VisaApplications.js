...
import { Search, Download, MoreVertical, CheckCircle, MinusCircle, Link } from 'lucide-react'
import { TbCaretUpDownFilled } from 'react-icons/tb'
import { IoFilterSharp } from 'react-icons/io5'
import { getVisaApplications, downloadVisaApplicationsCSV} from '@/services/visa/visa.service'
...
const toName = a => [a?.firstName, a?.middleName, a?.lastName].map(s => String(s || '').trim()).filter(Boolean).join(' ')
const toStatus = v => {
...
const TableHeaderCell = ({ children, onClick }) => (
  <button type='button' onClick={onClick} className='flex items-center gap-1 text-xs font-medium uppercase tracking-[0.12em] text-[#8A92AC] hover:text-[#2D3658]'>
    {children}
...
export default function VisaApplications () {
    ...
    const fetchData = async () => {
      ...
      try {
        ...
        const mapped = list.map(a => ({
          ...
          createdTs: (() => { const d = a.createdAt || a.updatedAt; const v = d && d.$date ? d.$date : d; const dt = v ? new Date(v) : null; return dt ? dt.getTime() : 0 })(),
          name: toName(a) || '-',
          email: a?.email || '-',
          phone: a?.mobile || '-',
  ...
  const filtered = useMemo(() => {
    ...
    const fmtCreated = d => {
      ...
      return date ? date.toLocaleString(undefined, { weekday: 'short', day: '2-digit', month: 'long', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '-'
    }
    return applications.filter(a => {
      ...
      const matchesText = name.includes(term) || email.includes(term) || phone.includes(term) || status.includes(term) || createdStr.includes(term)
    ...
    <div className='space-y-7 py-12 px-12'>
      <div className='flex flex-col gap-4 md:flex-row md:items-start md:justify-between'>
        <div className='flex flex-col gap-2'>
          <h1 className='text-2xl font-semibold text-slate-900'>Visa Applications</h1>
          <p className='text-sm text-[#99A1BC]'>Dashboard / Visa Applications</p>
        </div>
      ...
      <div className='grid grid-cols-1 md:grid-cols-3 gap-6 mb-8'>
        {cardDefs.map(card => (
          <div key={card.id} className={`${card.bg} rounded-2xl p-6 text-white relative overflow-hidden`}>
            <div className='flex items-center justify-between'>
              ...
              <div className='text-right'>
                ...
                <p className='text-4xl font-bold text-white'>{String(card.id === 'total' ? metrics.total : card.id === 'completed' ? metrics.completed : metrics.pending)}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className='rounded-[30px] border border-[#E1E6F7] bg-white p-6 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.55)]'>
        <div className='mb-6 flex flex-wrap items-center justify-between gap-4'>
          <h2 className='text-lg font-semibold text-slate-900'>Visa Applications List</h2>
          <div className='flex flex-wrap items-center gap-3'>
            {selectedIds.length > 0 && (
              <button onClick={markSelectedAsProcess} className='flex h-10 items-center gap-2 rounded-xl bg-[#FF5B2C] px-4 text-sm font-semibold text-white transition hover:bg-[#F0481A]'>
                Mark as process
              </button>
            )}
            ...
            <button onClick={downloadCsv} className='flex h-10 items-center gap-2 rounded-xl border border-[#E5E6EF] bg-white px-4 text-sm font-medium text-[#2D3658] transition hover:bg-[#F6F7FD]'>
        ...
        <div className='overflow-visible rounded-2xl border border-[#E5E8F5]'>
          <div className='grid grid-cols-[44px_1.5fr_2fr_2fr_1.5fr_1fr_1.2fr_60px] gap-3 bg-[#F7F9FD] px-6 py-4'>
            <div className='flex items-center'>
              <input type='checkbox' checked={isAllSelected} onChange={toggleSelectAll} className='h-4 w-4 rounded border-[#D0D5DD] text-[#FF5B2C] focus:ring-[#FF5B2C]' />
            </div>
            <div><TableHeaderCell onClick={() => toggleSort('date')}>Created On</TableHeaderCell></div>
            <div><TableHeaderCell onClick={() => toggleSort('name')}>User Name</TableHeaderCell></div>
            <div><TableHeaderCell onClick={() => toggleSort('email')}>Email</TableHeaderCell></div>
            <div><TableHeaderCell onClick={() => toggleSort('phone')}>Phone Number</TableHeaderCell></div>
            <div><TableHeaderCell>Action</TableHeaderCell></div>
          ...
          <div className='divide-y divide-[#EEF1FA] bg-white'>
            ...
            {!loading && !error && sorted.map((app, idx) => (
              <div key={app.id || idx} className='grid grid-cols-[44px_1.5fr_2fr_2fr_1.5fr_1fr_1.2fr_60px] gap-3 px-6 py-5 hover:bg-[#F9FAFD]'>
                <div className='self-center'>
                  <input type='checkbox' checked={isSelected(app.id)} onChange={() => toggleRowSelect(app.id)} className='h-4 w-4 rounded border-[#D0D5DD] text-[#FF5B2C] focus:ring-[#FF5B2C]' />
                </div>
                ...
                <div className='flex items-center gap-3'>
                  ...
                  <div className='min-w-0'>
                    <p className='text-sm font-medium text-slate-900 leading-tight'>{app.name}</p>
                    
                  </div>
                </div>
                <div className='self-center text-sm text-[#5E6582]'>{app.email}</div>
                <div className='self-center text-sm text-[#5E6582]'>{app.phone}</div>
                <div className='self-center text-sm text-[#5E6582]'>
                 <button className='text-[11px] font-semibold text-[#0F4EF1] hover:underline' onClick={() => router.push(`/visa/applications/${app.id}`)}>View Application</button>
                </div>
                <div className='flex items-center self-center'>
                  <span className={`inline-flex items-center justify-center rounded-full px-3 py-1 text-xs font-semibold ${statusClass(app.status)}`}>
                    {app.status}
                  </span>
                </div>
                <div className='flex items-center justify-center self-center relative'>
                  <button
                    onClick={() => setActiveDropdown(activeDropdown === (app.id || idx) ? null : (app.id || idx))}
                  ...
                  {activeDropdown === (app.id || idx) && (
                    <div ref={dropdownRef} className='absolute right-0 mt-2 w-52 rounded-xl border border-[#E5E8F5] bg-white shadow-[0_14px_30px_-20px_rgba(15,23,42,0.25)] z-50'>
                      <div className='py-2'>
                        <button className='block w-full text-left px-4 py-2 text-sm text-[#2D3658] hover:bg-[#F6F7FD]' onClick={() => router.push(`/visa/applications/${app.id}`)}>View Application</button>
                        <button className='block w-full text-left px-4 py-2 text-sm text-[#2D3658] hover:bg-[#F6F7FD]' onClick={() => markSelectedAsProcess()}>Mark as process</button>
