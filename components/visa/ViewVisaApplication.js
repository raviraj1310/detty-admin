...
export default function ViewVisaApplication({ applicationId }) {
  ...
  const prettyDate = (iso) => {
    try {
      ...
      return d ? d.toLocaleDateString(undefined, { day: '2-digit', month: 'long', year: 'numeric' }) : ''
    } catch { return '' }
  }
  ...
  const validateStep1 = (d) => {
    ...
    if (!String(d.passportExpiryDate || '').trim()) e.passportExpiryDate = 'Required'
    if (!String(d.countryOfResidence || '').trim()) e.countryOfResidence = 'Required'
  ...
  useEffect(() => {
    const fetchDetail = async () => {
      ...
      try {
        ...
        const docs = [
          { id: '1', title: 'UpInvitation letter from family/friend in Nigeria accepting immigration responsibility (IR)', url: toImageSrc(a.invitationLetter) },
          { id: '2', title: 'Copy of Nigerian Passport of the Host or Residency', url: toImageSrc(a.hostPassportOrPermit) },
          { id: '3', title: 'Valid Passport (not less than 6 months validity)', url: toImageSrc(a.validPassport) },
          { id: '4', title: 'Passport size photo', url: toImageSrc(a.passportPhoto) },
          { id: '5', title: 'Evidence of return ticket', url: toImageSrc(a.returnTicket) },
          { id: '6', title: 'Evidence of Hotel Reservations or host address in Nigeria (Verifiable Address).', url: toImageSrc(a.hotelOrHostAddressProof) },
          { id: '7', title: 'Evidence of sufficient funds, 180 days Bank Statement must be provided.', url: toImageSrc(a.bankStatement180Days) }
        ].filter(d => d.url)
        setDocuments(docs)
      } catch {}
    }
      ...
      <div className='rounded-[30px] border border-[#E1E6F7] bg-white p-6 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.55)]'>
        ...
        <div className='grid grid-cols-1 md:grid-cols-3 gap-x-8 gap-y-4'>
          <div>
            ...
            <div className='text-sm font-semibold text-slate-900'>{details.firstName}</div>
          ...
          <div>
            ...
            <div className='text-sm font-semibold text-slate-900'>{details.surname}</div>
            {errors.surname && <div className='text-xs text-red-600 mt-1'>{errors.surname}</div>}
          </div>
          <div>
            <div className='text-sm text-[#667085]'>Middle Name</div>
            <div className='text-sm font-semibold text-slate-900'>{details.middleName}</div>
          </div>
          ...
          <div>
            ...
            <div className='text-sm font-semibold text-slate-900'>{details.gender}</div>
          ...
          <div>
            ...
            <div className='text-sm font-semibold text-slate-900'>{details.passportNumber}</div>
            {errors.passportNumber && <div className='text-xs text-red-600 mt-1'>{errors.passportNumber}</div>}
          </div>

          <div>
            <div className='text-sm text-[#667085]'>Marital Status</div>
            <div className='text-sm font-semibold text-slate-900'>{details.maritalStatus}</div>
            {errors.maritalStatus && <div className='text-xs text-red-600 mt-1'>{errors.maritalStatus}</div>}
          </div>
          <div>
            <div className='text-sm text-[#667085]'>Passport Expiry Date (DD/MM/YY)</div>
            <div className='text-sm font-semibold text-slate-900'>{prettyDate(details.passportExpiryDate)}</div>
            {errors.passportExpiryDate && <div className='text-xs text-red-600 mt-1'>{errors.passportExpiryDate}</div>}
          </div>

          <div>
            <div className='text-sm text-[#667085]'>Country of Residence</div>
            <div className='text-sm font-semibold text-slate-900'>{details.countryOfResidence}</div>
            {errors.countryOfResidence && <div className='text-xs text-red-600 mt-1'>{errors.countryOfResidence}</div>}
          </div>
          <div>
            <div className='text-sm text-[#667085]'>City of Residence</div>
            <div className='text-sm font-semibold text-slate-900'>{details.cityOfResidence}</div>
            {errors.cityOfResidence && <div className='text-xs text-red-600 mt-1'>{errors.cityOfResidence}</div>}
          </div>
          <div>
            <div className='text-sm text-[#667085]'>Preferred Method of Mobility</div>
            <div className='text-sm font-semibold text-slate-900'>{details.mobilityMethod}</div>
            {errors.mobilityMethod && <div className='text-xs text-red-600 mt-1'>{errors.mobilityMethod}</div>}
          </div>
        </div>

        <div className='mt-6 border-t border-gray-200 pt-4'>
          <div className='inline-block rounded-md bg-black px-4 py-2 text-sm font-semibold text-white'>Uploaded Documents</div>
          <div className='mt-4 grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-4'>
            {documents.map(doc => (
              <div key={doc.id} className='space-y-2'>
                <div className='text-sm text-[#2D3658]'>{doc.title}</div>
                <a href={doc.url} target='_blank' rel='noopener noreferrer' className='text-sm font-semibold text-[#0F4EF1] hover:underline'>View Document</a>
              </div>
            ))}
          </div>
        </div>
