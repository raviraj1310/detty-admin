...
import { downloadSpaList } from '@/services/excel/excel.service'
...
export default function SpaMaster () {
  ...
  const handleDownloadSpaList = async () => {
    ...
    try {
      ...
      const blob = await downloadSpaList(params)
      if (!blob) {
      ...
      a.href = url
      a.download = `spa-list-${new Date().toISOString().slice(0, 10)}.xlsx`
  ...
  const fetchSpas = async () => {
    try {
      ...
      const response = await getAllSpa(
        pagination.page,
        pagination.limit,
        params
      )
      ...
      if (response.success) {
        ...
        setPagination({
          ...
          totalDocs: response.totalRecords,
  ...
  const handleEditSpaAccess = id => {
    router.push(`/spa/spa-access/${id}`)
  }
  ...
  const confirmDelete = async () => {
    ...
    setDeleting(true)
    try {
      const res = await deleteSpa(deleteId)
      if (res && res.success) {
        showToast('Spa deleted', 'The spa has been successfully deleted')
        fetchSpas()
      } else {
        showToast(
          'Error',
          res?.message || 'Failed to delete spa',
          'destructive'
        )
      }
    } catch (error) {
      console.error('Failed to delete spa:', error)
      const errMsg =
        error?.response?.data?.message ||
  ...
  const handleStatusChange = async (id, status) => {
    try {
      const res = await activeInactiveSpa(id, { status })
      if (res && res.success) {
        showToast(
          'Status updated',
          `Spa is now ${status ? 'Active' : 'Inactive'}`
        )
        fetchSpas()
      } else {
        showToast(
      ...
      console.error('Failed to update spa status:', error)
      showToast('Error', 'Failed to update spa status', 'destructive')
    }
    setActiveDropdown(null)
  }
    ...
    <div className='space-y-6 py-4 px-6'>
      ...
      <div className='flex flex-col gap-4 md:flex-row md:items-start md:justify-between'>
        <div className='flex flex-col gap-1'>
          <h1 className='text-xl font-semibold text-slate-900'>Spa Master</h1>
          <p className='text-xs text-[#99A1BC]'>Dashboard / Spa Master</p>
        </div>
      ...
      <div className='rounded-xl border border-[#E5E6EF] bg-white p-4 shadow-sm'>
        ...
        <div className='mb-4 flex flex-col gap-4 md:flex-row md:items-center md:justify-between'>
          <h2 className='text-base font-semibold text-slate-900'>Spa List</h2>
                      ...
                      >
                        <span
                          className={`mr-1 h-1.5 w-1.5 rounded-full ${
                            spa.status ? 'bg-emerald-500' : 'bg-red-500'
                          }`}
                        />
                        {spa.status ? 'Active' : 'Inactive'}
                      </span>
                    </td>
                    <td className='relative py-3 px-4 text-right'>
                      <button
                        onClick={e => handleDropdownClick(e, spa._id)}
                        className='rounded-lg p-1 text-gray-400 hover:bg-gray-100 hover:text-gray-600'
                      >
                        <MoreVertical className='h-4 w-4' />
                      </button>

                      {activeDropdown === spa._id && (
                        <div
                          ref={dropdownRef}
                          style={{
                            position: 'fixed',
                        ...
                        >
                          <button
                            onClick={() => router.push(`/spa/edit/${spa._id}`)}
                          ...
                          <button
                            onClick={() =>
                              router.push(`/spa/bookings/${spa._id}`)
                            }
                            className='flex w-full items-center gap-2 px-3 py-2 text-xs text-gray-600 hover:bg-gray-50'
                          >
                            View Bookings
                          ...
                          <button
                            onClick={() =>
                              router.push(`/spa/sessions/${spa._id}`)
                            }
                            className='flex w-full items-center gap-2 px-3 py-2 text-xs text-gray-600 hover:bg-gray-50'
                          >
                            View/Edit Spa Session
                          </button>
                          <button
                            onClick={() => handleDeleteSpa(spa._id)}
                          ...
                          {spa.status ? (
                            <button
                              onClick={() => handleStatusChange(spa._id, false)}
                              className='flex w-full items-center gap-2 px-3 py-2 text-xs text-red-600 hover:bg-red-50'
                            >
                              Inactive
                            </button>
                            ...
                            <button
                              onClick={() => handleStatusChange(spa._id, true)}
        ...
        <div className='mt-4 flex items-center justify-between border-t border-[#E5E6EF] pt-4'>
          <p className='text-xs text-gray-500'>
            Showing {spas.length} of {pagination.totalDocs} results
          </p>
      ...
      {/* Delete Confirmation Modal (Simple Alert for now or use a Modal component if available) */}
      {confirmOpen && (
        <div className='fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4'>
          <div className='w-full max-w-sm rounded-xl bg-white p-6 shadow-xl'>
            <div className='mb-4 flex items-center gap-3 text-red-600'>
              ...
              <h3 className='text-lg font-semibold'>Delete Spa?</h3>
            </div>
