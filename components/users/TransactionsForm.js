...
import { getBookingList, downloadBookingReceipt } from '../../services/booking/booking.service';
...
export default function TransactionsForm() {
    ...
    const formatEventDate = (dateInput, timeInput) => {
        const raw = dateInput && typeof dateInput === 'object' && dateInput.$date ? dateInput.$date : dateInput;
        ...
        const fetchData = async () => {
            ...
            try {
                ...
                const list = raw.map((b, idx) => ({
                    ...
                    eventName: b.event && (b.event.title || b.event.eventName) ? (b.event.title || b.event.eventName) : '-',
                    type: b.event && (b.event.eventType || b.event.type) ? (b.event.eventType || b.event.type) : '-',
                    ticketsBooked: `${typeof b.quantity === 'number' ? b.quantity : '-'} x ${b.ticketName || '-'}`,
                    ticketsQty: typeof b.quantity === 'number' ? b.quantity : 0,
                    additionalInfo: '',
                    amount: typeof b.totalPrice === 'number' ? `₦${b.totalPrice.toLocaleString()}` : '-',
                    amountNum: typeof b.totalPrice === 'number' ? b.totalPrice : (Number(b.totalPrice) || 0),
                    eventStatus: String(b.status || 'Pending'),
                    paymentStatus: String(b.paymentStatus || 'Pending'),
                    buyer: b.buyer || null,
                    event: b.event || null,
                    eventDateText: formatEventDate(b?.event?.eventStartDate, b?.event?.eventStartTime),
                    sortTs: Math.max(
    ...
    const filteredBookings = bookings.filter(booking => {
        ...
        const name = String(booking.eventName || booking?.raw?.event?.eventName || '').toLowerCase();
        const type = String(booking.type || '').toLowerCase();
        const dateStr = String(booking.eventDateText || booking.bookedOn || '').toLowerCase();
        const dateDigits = String(booking.eventDateText || booking.bookedOn || '').replace(/[^0-9]/g, '');
        const matchesText = name.includes(term) || type.includes(term) || dateStr.includes(term);
        ...
        const val = (key) => {
            switch (key) {
                ...
                case 'name':
                    return String(a.eventName || '').localeCompare(String(b.eventName || ''), undefined, { sensitivity: 'base' });
                case 'type':
                ...
                case 'eventStatus':
                    return String(a.eventStatus || '').localeCompare(String(b.eventStatus || ''), undefined, { sensitivity: 'base' });
                case 'paymentStatus':
                    return String(a.paymentStatus || '').localeCompare(String(b.paymentStatus || ''), undefined, { sensitivity: 'base' });
                default:
                    return a.sortTs - b.sortTs;
            }
        };
    ...
    const openTicket = (booking) => {
        ...
        if (id) {
            router.push(`/discover-events/tickets-booked/view/${encodeURIComponent(String(id))}${qs}`);
            setMenuOpenId(null);
            return;
        }
    ...
    const downloadReceipt = (booking) => {
        ...
        const eid = booking?.raw?.event?._id || booking?.raw?.event?.id || booking?.event?.id;
        const inferExt = type => {
        ...
        (async () => {
            try {
                ...
                const msg = e?.response?.data?.message || e?.message || 'Failed to download receipt';
                alert(msg);
            } finally {
                    ...
                    <table className="w-full">
                        <thead className="bg-gray-50 sticky top-0">
                            <tr>
                                ...
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    <button type="button" onClick={() => toggleSort('type')} className="flex items-center">
                                        <span>Type</span>
                                        <TbCaretUpDownFilled className="w-3 h-3 text-gray-400 ml-1" />
                                    </button>
                                </th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    <button type="button" onClick={() => toggleSort('tickets')} className="flex items-center">
                                        <span>Tickets Booked</span>
                                        <TbCaretUpDownFilled className="w-3 h-3 text-gray-400 ml-1" />
                                    </button>
                                </th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    <button type="button" onClick={() => toggleSort('amount')} className="flex items-center">
                                        <span>Amount</span>
                                        <TbCaretUpDownFilled className="w-3 h-3 text-gray-400 ml-1" />
                                    </button>
                                </th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    <button type="button" onClick={() => toggleSort('eventStatus')} className="flex items-center">
                                        <span>Event Status</span>
                                        <TbCaretUpDownFilled className="w-3 h-3 text-gray-400 ml-1" />
                                    </button>
                                </th>
                                <th className="px-4 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">
                                    <button type="button" onClick={() => toggleSort('paymentStatus')} className="flex items-center">
                                        <span>Payment Status</span>
                                        <TbCaretUpDownFilled className="w-3 h-3 text-gray-400 ml-1" />
                                    </button>
                                </th>
                                <th className="px-8 py-3 text-right text-xs font-medium text-gray-500 uppercase tracking-wider w-20">
    ...
    <tr key={booking.id} className="hover:bg-gray-50">
      ...
      <td className="px-4 py-4 whitespace-nowrap text-sm text-gray-900">
        ...
        <div className="text-xs text-gray-500">{booking.additionalInfo}</div>
      </td>
          ...
          {menuOpenId === booking.id && (
            ...
            >
              ...
              >
                ...
                <span className="text-gray-800">View Ticket</span>
              </button>
              ...
              >
                <span className="mr-3 text-gray-500">
                  <svg className="h-4 w-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5.121 17.804A2 2 0 016.999 17h10.002a2 2 0 011.878.804M15 11a3 3 0 11-6 0 3 3 0 016 0z" />
                  </svg>
                </span>
              ...
              >
                <span className="mr-3 text-gray-500">
                  {String(downloadingId || '') === String(booking.id) ? (
                ...
                <span className="text-gray-800">Download Ticket</span>
              </button>
            </div>
          )}
        </div>
    ...
    <td colSpan="8" className="px-4 py-4 text-center text-sm text-gray-500">
      No bookings found.
        ...
        {ticketOpen && selectedBooking && (
            <Modal
                open={ticketOpen}
                onOpenChange={v => { if (!v) { setTicketOpen(false); setSelectedBooking(null); } }}
                title={'Ticket'}
            >
                <div className='space-y-2 text-sm text-[#2D3658]'>
                    <div className='flex justify-between'><span>Booking ID</span><span>{selectedBooking._id || selectedBooking.id || selectedBooking.bookingId || '-'}</span></div>
                    <div className='flex justify-between'><span>Booked On</span><span>{(() => { const d = selectedBooking.bookedOn || selectedBooking.createdAt || selectedBooking.updatedAt; const date = d ? new Date(typeof d === 'object' && d.$date ? d.$date : d) : null; return date ? date.toLocaleString() : '-'; })()}</span></div>
                    <div className='flex justify-between'><span>Ticket</span><span>{selectedBooking.ticketsBooked || selectedBooking.ticketName || '-'}</span></div>
                    <div className='flex justify-between'><span>Amount</span><span>{typeof selectedBooking.totalPrice === 'number' ? `₦${selectedBooking.totalPrice.toLocaleString()}` : selectedBooking.amount || '-'}</span></div>
                </div>
                ...
                <div className='space-y-6'>
                    <div className='rounded-xl bg-[#F8F9FC] p-4'>
                        <div className='grid grid-cols-2 gap-4 text-sm'>
                            ...
                            <div className='text-right font-semibold text-slate-900'>{selectedBooking?.buyer?.fullName || '-'}</div>
                            <div className='text-[#5E6582]'>Email Address</div>
                            <div className='text-right font-semibold text-slate-900'>{selectedBooking?.buyer?.email || '-'}</div>
                            <div className='text-[#5E6582]'>Phone</div>
                            <div className='text-right font-semibold text-slate-900'>{selectedBooking?.buyer?.phone || '-'}</div>
                        </div>
                    </div>
                </div>
            </Modal>
