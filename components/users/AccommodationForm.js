  ...
  const filteredAccommodations = useMemo(
    () =>
      rows.filter(accommodation => {
        ...
        const bookingsStr = String(accommodation.bookings || '').toLowerCase()
        const bookingsDigits = bookingsStr.replace(/[^0-9]/g, '')
        const amountStr = String(accommodation.amount || '').toLowerCase()
        ...
        const matchesText =
          ...
          bookingsStr.includes(term) ||
          amountStr.includes(term)

        const matchesDigits =
          (termDigits && addedDigits.includes(termDigits)) ||
          bookingsDigits.includes(termDigits) ||
