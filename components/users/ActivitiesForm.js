...
export default function Activities() {
  ...
  useEffect(() => {
    const fetchBookings = async () => {
      ...
      try {
        ...
        const res = await getActivityBookedTickets(undefined, params);
        let data = [];
        ...
        const mapped = data.map((b, idx) => {
          ...
          const arrivalDate = formatEventDateWthOutTime(b.arrivalDate);
  ...
  const filteredActivities = rows.filter((r) => {
    ...
    if (dateRange.start) {
      ...
      if (bookedDate && bookedDate < start) matchesDate = false;
    }
    if (dateRange.end && matchesDate) {
      const end = new Date(dateRange.end + "T23:59:59.999");
      if (bookedDate && bookedDate > end) matchesDate = false;
    }
    ...
    const arrivalDate = String(r.arrivalDate || "").toLowerCase();
  ...
  return (
    <div className="p-4 min-h-screen bg-white overflow-x-hidden">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-6">
        {/* Total Activity Bookings (Filtered) */}
          ...
          <div className="p-4 border-b border-gray-200">
            <div className="flex justify-between items-center mb-3">
              <h2 className="text-lg font-semibold text-gray-900">
                Gross Transaction Value of Activities
              </h2>
