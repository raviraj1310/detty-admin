...
export default function ProductOrders() {
  ...
  const handleDownloadProductExcel = () => {
    try {
      setExporting(true);
      ...
      rawData.forEach((order) => {
        const buyer = order.userId || {}; // Buyer info
        ...
        (order.items || []).forEach((item) => {
          ...
          dataToExport.push({
            ...
            Category_Title: product.categoryId?.title || "",
            Price: product.price,
            Stock: product.stock,
            ...
            Updated_At: formatDate(product.updatedAt),

            /* ===== Order Info ===== */
            Order_ID: orderId,
            Quantity: item.quantity,
            Item_Price: item.price,
            Total_Amount: order.totalAmount || item.price * item.quantity,
            Buyer_Name: buyer.name || order.userName || "",
            Buyer_Email: buyer.email || order.email || "",
            Buyer_Phone: buyer.phoneNumber || "",
            Order_Status: order.status,
            Payment_Status: order.paymentStatus,
            Booked_On: formatDate(order.createdAt),
            ...
            Final_Payable_Amount: order.finalPayableAmount || 0,
          });
        });
      });

      downloadExcel(dataToExport, `Product_Orders.xlsx`);
    } finally {
      setExporting(false);
    }
  };
