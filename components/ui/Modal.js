    ...
    <div className="fixed inset-0 z-40 flex items-center justify-center">
      ...
      <div className="relative z-50 w-full max-w-lg rounded-2xl border border-[#E5E8F6] bg-white p-6 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.55)]">
        <div className="flex items-center justify-between mb-4">
          {title ? <div className="text-lg font-semibold text-slate-900">{title}</div> : <div />}
          <button onClick={() => onOpenChange(false)} className="rounded-full p-2 text-[#8C93AF] hover:bg-[#F5F7FD] hover:text-[#2D3658]">
            <X className="h-5 w-5" />
          </button>
        </div>
        <div>{children}</div>
