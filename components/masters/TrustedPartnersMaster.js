...
export default function TrustedPartnersMaster() {
  ...
  const [formData, setFormData] = useState({ name: "", uploadImage: "", status: "Active", imageFile: null });
  ...
  const fetchPartners = useCallback(async () => {
    ...
    try {
      ...
      const normalizedPartners = (response?.data ?? []).map((partner) => ({
        ...
        addedOn: partner.createdAt ? new Date(partner.createdAt).toLocaleString(undefined, { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'NA',
        ...
        statusClass: partner.status ? "bg-emerald-50 text-emerald-600 border border-emerald-200" : "bg-red-50 text-red-600 border border-red-200",
      }));
  ...
  useEffect(() => {
    ...
    fetchPartners();
    return () => { isMountedRef.current = false; if (previewUrlRef.current) URL.revokeObjectURL(previewUrlRef.current); };
  }, [fetchPartners]);

  useEffect(() => {
    const handleClickOutside = (event) => {
      if (dropdownRef.current && !dropdownRef.current.contains(event.target)) setMenuOpenId(null);
    };
  ...
  const updateImagePreview = (file) => {
    ...
    if (file) { const preview = URL.createObjectURL(file); previewUrlRef.current = preview; setImagePreview(preview); }
    else setImagePreview(null);
  };

  const handleFileChange = (event) => {
    const file = event.target.files?.[0];
    setFormData((prev) => ({ ...prev, imageFile: file || null, uploadImage: file?.name || "" }));
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setFormData({ name: "", uploadImage: "", status: "Active", imageFile: null });
      updateImagePreview(null);
      await fetchPartners();
    } catch (error) {
      console.error("Failed to create partner:", error);
      setPartnersError(error?.response?.data?.message || "Unable to create partner. Please try again.");
    } finally {
  ...
  const getSortValue = (p, key) => {
    if (key === 'addedOn') { const d = p.createdAtRaw; return d ? new Date(d).getTime() : 0; }
  ...
  const startEdit = async (id) => {
    ...
    try {
      ...
      if (p) {
        const appendCacheBuster = (url, token) => { if (!url) return ''; const sep = url.includes('?') ? '&' : '?'; return url + sep + 't=' + token; };
        const ts = p.createdAtRaw ? new Date(p.createdAtRaw).getTime() : Date.now();
        setFormData({ name: String(p.partnerName || ''), uploadImage: String(p.partnerLogo || ''), status: p.status === 'Active' ? 'Active' : 'Inactive', imageFile: null });
  ...
  const confirmDelete = async () => {
    if (!confirmId) return;
    setDeleting(true);
    try { await deletePartner(confirmId); await fetchPartners(); }
    catch { setPartnersError('Failed to delete partner'); }
    ...
    <div className="space-y-5 py-2 px-2">
      <div className="flex flex-col gap-1 md:flex-row md:items-start md:justify-between">
        <div className="flex flex-col gap-1">
          <h1 className="text-xl font-semibold text-slate-900">Trusted Partners Master</h1>
      ...
      <div className="bg-gray-100 rounded-xl p-2">
        <div className="rounded-xl border border-[#E1E6F7] bg-white p-5 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.55)]">
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-sm font-semibold text-slate-900">Trusted Partners Details</h2>
            <button type="submit" form={formId} disabled={isSubmitting} className="rounded-xl bg-[#FF5B2C] px-4 py-2 text-xs font-semibold text-white shadow-[0_14px_30px_-20px_rgba(248,113,72,0.65)] transition hover:bg-[#F0481A] disabled:cursor-not-allowed disabled:opacity-70">
          ...
          <form id={formId} onSubmit={handleSubmit} ref={formSectionRef}>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <div className="space-y-1">
                ...
                <input type="text" value={formData.name} onChange={(e) => handleInputChange("name", e.target.value)} ref={nameInputRef} className="w-full h-9 rounded-lg border border-[#E5E6EF] bg-[#F8F9FC] px-3 text-xs text-slate-700 placeholder:text-[#B0B7D0] focus:border-[#C5CAE3] focus:outline-none focus:ring-2 focus:ring-[#C2C8E4]" placeholder="Enter partner name" />
                {formErrors.name && <p className="text-xs text-red-500">{formErrors.name}</p>}
              </div>

              <div className="space-y-1">
                <label className="text-xs font-medium text-slate-700">Upload Image*</label>
                <div className="flex h-9 items-stretch overflow-hidden rounded-lg border border-[#E5E6EF]" onClick={handleBrowse}>
                  ...
                  <button type="button" onClick={handleBrowse} className="px-3 text-xs font-medium text-[#2D3658] bg-white transition hover:bg-[#F6F7FD]">Browse</button>
                </div>
                <input id="partner-image-upload" type="file" accept="image/*" ref={fileInputRef} className="hidden" onChange={handleFileChange} />
                {(imagePreview || formData.uploadImage) && (
                  <div className="rounded-lg border border-dashed border-[#E5E6EF] bg-[#F8F9FC] p-2">
                    <p className="text-[10px] font-medium text-[#5E6582] mb-1">Preview</p>
                    <div className="h-12 w-12 overflow-hidden rounded-lg border border-[#E5E6EF] bg-white">
                      <img src={imagePreview || buildPartnerImageUrl(formData.uploadImage)} alt="Selected partner" className="h-full w-full object-cover" />
                    </div>
                  </div>
                )}
      ...
      <div className="bg-gray-100 rounded-xl p-2">
        <div className="rounded-xl border border-[#E1E6F7] bg-white p-4">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
            <h2 className="text-sm font-semibold text-slate-900">Trusted Partners</h2>
          ...
          <div className="overflow-hidden rounded-xl border border-[#E5E8F5]">
            <div className="grid grid-cols-12 gap-4 bg-[#F7F9FD] px-4 py-3">
              ...
              <div className="col-span-6"><TableHeaderCell onClick={() => toggleSort('partnerName')} active={sortKey === 'partnerName'} order={sortOrder}>Trusted Partners</TableHeaderCell></div>
              <div className="col-span-2"><TableHeaderCell align='right' onClick={() => toggleSort('status')} active={sortKey === 'status'} order={sortOrder}>Status</TableHeaderCell></div>
              <div className="col-span-1"><TableHeaderCell align='right'></TableHeaderCell></div>
            </div>

            <div className="divide-y divide-[#EEF1FA] bg-white">
              {isLoadingPartners && <div className="px-4 py-4 text-center text-xs text-[#5E6582]">Loading partners...</div>}
              {!isLoadingPartners && partnersError && <div className="px-4 py-4 text-center text-xs text-red-600">{partnersError}</div>}
              {!isLoadingPartners && !partnersError && filteredTrustedPartners.length === 0 && <div className="px-4 py-4 text-center text-xs text-[#5E6582]">No partners found.</div>}

              {!isLoadingPartners && !partnersError && sortedTrustedPartners.map((partner) => (
                <div key={partner.id} className="grid grid-cols-12 gap-4 px-4 py-3 hover:bg-[#F9FAFD]">
                  <div className="col-span-3 self-center text-xs text-[#5E6582] whitespace-nowrap">{partner.addedOn}</div>
                  <div className="col-span-6 flex items-center gap-2 self-center">
                    <div className="w-10 h-10 rounded-lg overflow-hidden border border-[#E5E6EF] bg-white shrink-0 flex items-center justify-center">
                      <img src={partner.partnerLogo} alt={partner.partnerName} className="w-full h-full object-contain p-0.5" />
                    </div>
                    <div className="flex flex-col">
                      <span className="text-xs font-semibold text-[#2D3658] uppercase">{partner.partnerName}</span>
                      <span className="text-[10px] text-[#8A92AC]">{partner.description}</span>
                    </div>
                  </div>
                  <div className="col-span-2 self-center">
                    <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${partner.statusClass}`}>{partner.status}</span>
                  </div>
                    ...
                    <div className='relative'>
                      <button onClick={e => {
                        if (menuOpenId === partner.id) setMenuOpenId(null);
                          ...
                          setMenuPos({ top: Math.round(rect.bottom + 6), left: Math.round(rect.right - 128) });
                      ...
                      {menuOpenId === partner.id && (
                        <div ref={dropdownRef} className='fixed w-32 rounded-md border border-[#E5E8F6] bg-white shadow-lg z-50' style={{ top: menuPos.top, left: menuPos.left }}>
                          <button onClick={() => startEdit(partner.id)} className='flex w-full items-center gap-2 px-3 py-1.5 text-xs text-[#2D3658] hover:bg-[#F6F7FD]' disabled={rowActionLoading === partner.id}>
                            {rowActionLoading === partner.id ? <Loader2 className='h-3.5 w-3.5 animate-spin' /> : <Pencil className='h-3.5 w-3.5' />}Edit
                          </button>
                          <button onClick={() => { setConfirmId(partner.id); setConfirmOpen(true); setMenuOpenId(null); }} className='flex w-full items-center gap-2 px-3 py-1.5 text-xs text-red-600 hover:bg-red-50'>
          ...
          <div className='relative z-50 w-full max-w-sm rounded-xl border border-[#E5E8F6] bg-white p-5 shadow-lg'>
            <div className='flex items-start gap-3'>
              ...
              <div className='flex-1'>
                <div className='text-sm font-semibold text-slate-900'>Delete this partner?</div>
