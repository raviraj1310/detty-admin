...
import { getAllCountries, getCountryById, createCountry, updateCountry, deleteCountry } from '@/services/country/country.service'
...
export default function CountryMaster() {
  ...
  const normalizeCountry = (d) => ({ _id: d?._id || d?.id || '', name: d?.name || '', createdAt: d?.createdAt || d?.updatedAt || '' })

  const handleInputChange = (field, value) => { setFormData((prev) => ({ ...prev, [field]: value })) }

  const validate = () => {
    const errs = {}
    if (!formData.name || formData.name.trim().length < 2) errs.name = 'Enter a valid country name'
    return errs
  }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({})
      setEditingId(null)
      setToast({ open: true, title: editingId ? 'Country updated' : 'Country created', description: editingId ? 'Changes have been saved' : 'Your country has been added', variant: 'success' })
    } catch (e) {
      setError(editingId ? 'Failed to update country' : 'Failed to create country')
      setToast({ open: true, title: 'Error', description: e?.response?.data?.message || e?.message || (editingId ? 'Failed to update country' : 'Failed to create country'), variant: 'error' })
    } finally {
      setSubmitting(false)
    }
  }
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await fetchCountries()
      setToast({ open: true, title: 'Country deleted', description: 'The country has been removed', variant: 'success' })
    } catch (e) {
  ...
  const filteredCountries = useMemo(() => {
    ...
    return base.filter((country) => String(country.name || '').toLowerCase().includes(term))
    ...
    <div className='space-y-5 py-2 px-3'>
      ...
      <div className='flex flex-col gap-1 md:flex-row md:items-start md:justify-between'>
        <div className='flex flex-col gap-1'>
          <h1 className='text-xl font-semibold text-slate-900'>Country Master</h1>
      ...
      <div className='bg-gray-100 rounded-xl p-2'>
        <div className='rounded-xl border border-[#E1E6F7] bg-white p-5 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.55)]'>
          <div className='flex items-center justify-between mb-4'>
            <h2 className='text-sm font-semibold text-slate-900'>Country Details</h2>
          ...
          <form onSubmit={handleSubmit} ref={formSectionRef}>
            <div className='space-y-1'>
              <label className='text-xs font-medium text-slate-700'>Country Name</label>
              <input type='text' ref={nameInputRef} value={formData.name} onChange={(e) => handleInputChange('name', e.target.value)} className='w-full h-9 rounded-lg border border-[#E5E6EF] bg-[#F8F9FC] px-3 text-xs text-slate-700 placeholder:text-[#B0B7D0] focus:border-[#C5CAE3] focus:outline-none focus:ring-2 focus:ring-[#C2C8E4]' placeholder='Enter country name' />
      ...
      <div className='bg-gray-100 rounded-xl p-2'>
        <div className='rounded-xl border border-[#E1E6F7] bg-white p-4'>
          <div className='mb-4 flex flex-wrap items-center justify-between gap-2'>
            <h2 className='text-sm font-semibold text-slate-900'>Country List</h2>
          ...
          <div className='overflow-hidden rounded-xl border border-[#E5E8F5]'>
            <div className='grid grid-cols-12 gap-4 bg-[#F7F9FD] px-4 py-3'>
              <div className='col-span-5'><TableHeaderCell onClick={() => toggleSort('addedOn')} active={sortKey === 'addedOn'} order={sortOrder}>Added On</TableHeaderCell></div>
              <div className='col-span-5'><TableHeaderCell onClick={() => toggleSort('name')} active={sortKey === 'name'} order={sortOrder}>Country Name</TableHeaderCell></div>
            ...
            <div className='divide-y divide-[#EEF1FA] bg-white'>
              ...
              {!loading && !error && sortedCountries.length === 0 && <div className='px-4 py-3 text-center text-xs text-[#5E6582]'>No countries found</div>}
              {!loading && !error && sortedCountries.map((country, idx) => (
                <div key={country._id || idx} className='grid grid-cols-12 gap-4 px-4 py-3 hover:bg-[#F9FAFD]'>
                  <div className='col-span-5 self-center text-xs text-[#5E6582] whitespace-nowrap'>
                    {country.createdAt ? new Date(country.createdAt).toLocaleString(undefined, { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '-'}
                  </div>
                  <div className='col-span-5 self-center text-xs font-semibold text-slate-900'>{country.name || '-'}</div>
                  <div className='col-span-2 flex items-center justify-end'>
                    <div className='relative'>
                      <button data-menu-button onClick={() => setMenuOpenId(menuOpenId === (country._id || idx) ? null : country._id || idx)} className='rounded-full border border-transparent p-1.5 text-[#8C93AF] transition hover:border-[#E5E8F6] hover:bg-[#F5F7FD] hover:text-[#2D3658]'>
                      ...
                      {menuOpenId === (country._id || idx) && (
                        <div data-menu-content className='absolute right-0 mt-1 w-32 rounded-md border border-[#E5E8F6] bg-white shadow-lg z-20'>
                          <button onClick={() => startEdit(country._id)} className='flex w-full items-center gap-2 px-3 py-1.5 text-xs text-[#2D3658] hover:bg-[#F6F7FD]' disabled={rowActionLoading === country._id}>
                            {rowActionLoading === country._id ? <Loader2 className='h-3.5 w-3.5 animate-spin' /> : <Pencil className='h-3.5 w-3.5' />}Edit
                          </button>
                          <button onClick={() => { setConfirmId(country._id); setConfirmOpen(true); setMenuOpenId(null) }} className='flex w-full items-center gap-2 px-3 py-1.5 text-xs text-red-600 hover:bg-red-50'>
            ...
            <div className='relative z-50 w-full max-w-sm rounded-xl border border-[#E5E8F6] bg-white p-5 shadow-lg'>
              <div className='flex items-start gap-3'>
                ...
                <div className='flex-1'>
                  <div className='text-sm font-semibold text-slate-900'>Delete this country?</div>
