...
import { getMerchandiseCategories, getMerchandiseCategoryById, createMerchandiseCategory, updateMerchandiseCategory, deleteMerchandiseCategory } from '@/services/merchandise/category.service'
    ...
    <div className='space-y-5 py-3 px-3'>
      ...
      <div className='flex flex-col gap-1 md:flex-row md:items-start md:justify-between'>
        <div className='flex flex-col gap-1'>
          <h1 className='text-xl font-semibold text-slate-900'>Merchandise Category Master</h1>
      ...
      <div className='bg-gray-100 p-2 rounded-xl'>
        <div className='rounded-xl border border-[#E1E6F7] bg-white p-5 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.55)]'>
          <div className='flex items-center justify-between mb-4'>
            <h2 className='text-sm font-semibold text-slate-900'>Merchandise Category Details</h2>
          ...
          <form onSubmit={handleSubmit} ref={formSectionRef}>
            <div className='grid grid-cols-1 md:grid-cols-3 gap-4'>
              <div className='space-y-1'>
                <label className='text-xs font-medium text-slate-700'>Merchandise Category Name</label>
                <input type='text' ref={nameInputRef} value={formData.title} onChange={e => handleInputChange('title', e.target.value)} className='w-full h-9 rounded-lg border border-[#E5E6EF] bg-[#F8F9FC] px-3 text-xs text-slate-700 placeholder:text-[#B0B7D0] focus:border-[#C5CAE3] focus:outline-none focus:ring-2 focus:ring-[#C2C8E4]' placeholder='Enter merchandise category name' />
      ...
      <div className='rounded-2xl border border-[#E1E6F7] bg-white p-4'>
        <div className='mb-4 flex flex-wrap items-center justify-between gap-2'>
          <h2 className='text-sm font-semibold text-slate-900'>Merchandise Category List</h2>
        ...
        <div className='overflow-visible rounded-xl border border-[#E5E8F5]'>
          <div className='grid grid-cols-12 gap-4 bg-[#F7F9FD] px-4 py-3'>
            ...
            <div className='col-span-4'><TableHeaderCell onClick={() => toggleSort('category')} active={sortKey === 'category'} order={sortOrder}>Category</TableHeaderCell></div>
          ...
          <div className='divide-y divide-[#EEF1FA] bg-white'>
            ...
            {!loading && !error && sortedCategories.map((c, idx) => (
              <div key={c._id || idx} className='grid grid-cols-12 gap-4 px-4 py-3 hover:bg-[#F9FAFD]'>
                <div className='col-span-4 self-center text-xs text-[#5E6582]'>
                  {c.createdAt ? new Date(c.createdAt).toLocaleString(undefined, { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '-'}
                </div>
                <div className='col-span-4 self-center text-xs font-semibold text-slate-900'>{c.title || '-'}</div>
            ...
            {!loading && !error && filteredCategories.length === 0 && <div className='px-4 py-4 text-center text-xs text-[#5E6582]'>No categories found</div>}
          </div>
        </div>
      </div>
          ...
          <div className='relative z-50 w-full max-w-sm rounded-xl border border-[#E5E8F6] bg-white p-5 shadow-lg'>
            <div className='flex items-start gap-3'>
              ...
              <div className='flex-1'>
                <div className='text-sm font-semibold text-slate-900'>Delete this category?</div>
