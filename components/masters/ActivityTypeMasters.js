...
import {
  ...
  updateActivityType,
  deleteActivityType,
  changeActivityTypeStatus,
} from "@/services/places-to-visit/activityType.service";
...
export default function ActivityTypeMasters() {
  ...
  const [activityDiscounts, setActivityDiscounts] = useState([]);
  ...
  const validate = () => {
    const errs = {};
    if (
      !formData.activityTypeName ||
      formData.activityTypeName.trim().length < 2
    )
      errs.activityTypeName = "Enter a valid activity type";
      ...
      if (editingTypeId) {
        ...
        if (res?.success) {
          setToast({
            ...
            description: formData.activityTypeName,
            variant: "success",
          });
  ...
  const filteredActivityTypes = useMemo(() => {
    ...
    return base.filter((a) => {
      ...
      const matchesText = !term
        ? true
        : name.includes(term) || addedStr.includes(term);
  ...
  const sortedActivityTypes = useMemo(() => {
    const arr = Array.isArray(filteredActivityTypes)
  ...
  const handleActivityStatusChange = async (id, currentStatus) => {
    setRowActionLoading(id);
    try {
      const newStatus = !currentStatus;
      await changeActivityTypeStatus(id, newStatus); // your service call
      setActivityDiscounts((prev) =>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              ...
              <input
                ...
                onChange={(e) =>
                  handleInputChange("activityTypeName", e.target.value)
                }
                ref={nameInputRef}
            ...
            <div className="space-y-1">
              ...
              <input
                ...
                onChange={(e) =>
                  handleInputChange("activityFor", e.target.value)
                }
                className="w-full h-9 rounded-lg border border-[#E5E6EF] bg-[#F8F9FC] px-3 text-xs text-slate-700 placeholder:text-[#B0B7D0] focus:border-[#C5CAE3] focus:outline-none focus:ring-2 focus:ring-[#C2C8E4]"
            ...
            <div className="space-y-1 md:col-span-2">
              ...
              <textarea
                ...
                rows={2}
                className="w-full rounded-lg border border-[#E5E6EF] bg-[#F8F9FC] px-3 py-2 text-xs text-slate-700 placeholder:text-[#B0B7D0] focus:border-[#C5CAE3] focus:outline-none focus:ring-2 focus:ring-[#C2C8E4]"
      ...
      <div className="rounded-2xl border border-[#E1E6F7] bg-white p-4">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          ...
          <div className="flex items-center gap-2">
            ...
            <select
              ...
              className="h-8 rounded-lg border border-[#E5E6EF] bg-white px-3 text-xs text-slate-700"
            >
        ...
        <div className="overflow-visible rounded-xl border border-[#E5E8F5]">
          <div className="grid grid-cols-12 gap-4 bg-[#F7F9FD] px-4 py-3">
            ...
            <div className="col-span-4">
              <TableHeaderCell
                onClick={() => toggleSort("activityTypeName")}
                active={sortKey === "activityTypeName"}
                order={sortOrder}
              >
                Activity Type
              </TableHeaderCell>
          ...
          <div className="divide-y divide-[#EEF1FA] bg-white">
            ...
            {!loading &&
              !error &&
              sortedActivityTypes.map((activityType, idx) => (
                <div
