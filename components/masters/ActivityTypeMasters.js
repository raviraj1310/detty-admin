...
const TableHeaderCell = ({ children, align = 'left', onClick, active = false, order = 'desc' }) => (
  <button
    type='button'
    onClick={onClick}
    className={`flex items-center gap-1 text-xs font-medium uppercase tracking-wide whitespace-nowrap ${align === 'right' ? 'justify-end' : 'justify-start'} ${active ? 'text-[#2D3658]' : 'text-[#8A92AC]'}`}
  >
    {children}
...
export default function ActivityTypeMasters() {
  ...
  const getSortValue = (a, key) => {
    ...
    if (key === 'activityTypeName') return String(a.activityTypeName || '').toLowerCase();
    if (key === 'status') return a.status ? 1 : 0;
    return 0;
  };
    ...
    <div className="space-y-5 py-6 px-6">
      <div className="flex flex-col gap-1 md:flex-row md:items-start md:justify-between">
        <div className="flex flex-col gap-1">
          <h1 className="text-xl font-semibold text-slate-900">Activity Type Masters</h1>
      ...
      <div className="rounded-2xl border border-[#E1E6F7] bg-white p-5 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.55)]">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-sm font-semibold text-slate-900">Activity Type Details</h2>
          <button onClick={handleSubmit} disabled={submitting} className="rounded-xl bg-[#FF5B2C] px-4 py-2 text-xs font-semibold text-white shadow-[0_14px_30px_-20px_rgba(248,113,72,0.65)] transition hover:bg-[#F0481A] disabled:opacity-60 disabled:cursor-not-allowed">
            {submitting ? (<span className="flex items-center gap-2"><Loader2 className="h-3.5 w-3.5 animate-spin" />{editingTypeId ? 'Updating...' : 'Adding...'}</span>) : editingTypeId ? 'Update' : 'Add'}
          </button>
        </div>
        ...
        <form onSubmit={handleSubmit} ref={formSectionRef}>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div className="space-y-1">
              <label className="text-xs font-medium text-slate-700">Activity Types Name</label>
              <input type="text" value={formData.activityTypeName} onChange={(e) => handleInputChange('activityTypeName', e.target.value)} ref={nameInputRef} className="w-full h-9 rounded-lg border border-[#E5E6EF] bg-[#F8F9FC] px-3 text-xs text-slate-700 placeholder:text-[#B0B7D0] focus:border-[#C5CAE3] focus:outline-none focus:ring-2 focus:ring-[#C2C8E4]" placeholder="Enter activity type name" />
              {errors.activityTypeName && <p className="text-xs text-red-600">{errors.activityTypeName}</p>}
            </div>
            <div className="space-y-1">
            ...
            <div className="space-y-1">
              <label className="text-xs font-medium text-slate-700">Activity Label</label>
              <input type="text" value={formData.activityFor} onChange={(e) => handleInputChange('activityFor', e.target.value)} className="w-full h-9 rounded-lg border border-[#E5E6EF] bg-[#F8F9FC] px-3 text-xs text-slate-700 placeholder:text-[#B0B7D0] focus:border-[#C5CAE3] focus:outline-none focus:ring-2 focus:ring-[#C2C8E4]" placeholder="Enter activity audience" />
            </div>
            <div className="space-y-1">
              <label className="text-xs font-medium text-slate-700">Title</label>
              <input type="text" value={formData.title} onChange={(e) => handleInputChange('title', e.target.value)} className="w-full h-9 rounded-lg border border-[#E5E6EF] bg-[#F8F9FC] px-3 text-xs text-slate-700 placeholder:text-[#B0B7D0] focus:border-[#C5CAE3] focus:outline-none focus:ring-2 focus:ring-[#C2C8E4]" placeholder="Enter title" />
            </div>
            <div className="space-y-1 md:col-span-2">
              <label className="text-xs font-medium text-slate-700">Description</label>
              <textarea value={formData.description} onChange={(e) => handleInputChange('description', e.target.value)} rows={2} className="w-full rounded-lg border border-[#E5E6EF] bg-[#F8F9FC] px-3 py-2 text-xs text-slate-700 placeholder:text-[#B0B7D0] focus:border-[#C5CAE3] focus:outline-none focus:ring-2 focus:ring-[#C2C8E4]" placeholder="Enter description" />
            </div>
          </div>
        </form>
      </div>
      ...
      <div className="rounded-2xl border border-[#E1E6F7] bg-white p-4">
        <div className="mb-4 flex flex-wrap items-center justify-between gap-2">
          <h2 className="text-sm font-semibold text-slate-900">Activity Type List</h2>
          <div className="flex items-center gap-2">
            ...
            <select value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)} className="h-8 rounded-lg border border-[#E5E6EF] bg-white px-3 text-xs text-slate-700">
        ...
        <div className="overflow-visible rounded-xl border border-[#E5E8F5]">
          <div className="grid grid-cols-12 gap-4 bg-[#F7F9FD] px-4 py-3">
            ...
            <div className="col-span-4"><TableHeaderCell onClick={() => toggleSort('activityTypeName')} active={sortKey === 'activityTypeName'} order={sortOrder}>Activity Type</TableHeaderCell></div>
            <div className="col-span-4"><TableHeaderCell align='right' onClick={() => toggleSort('status')} active={sortKey === 'status'} order={sortOrder}>Status</TableHeaderCell></div>
          </div>
          ...
          <div className="divide-y divide-[#EEF1FA] bg-white">
            ...
            {!loading && !error && sortedActivityTypes.map((activityType, idx) => (
              <div key={activityType._id || idx} className="grid grid-cols-12 gap-4 px-4 py-3 hover:bg-[#F9FAFD]">
                <div className="col-span-4 self-center text-xs text-[#5E6582]">
                  {activityType.createdAt || activityType.updatedAt ? new Date(activityType.createdAt || activityType.updatedAt).toLocaleString(undefined, { weekday: 'short', day: '2-digit', month: 'short', year: 'numeric', hour: '2-digit', minute: '2-digit' }) : '-'}
                </div>
                <div className="col-span-4 self-center text-xs font-semibold text-slate-900">{activityType.activityTypeName || '-'}</div>
                <div className="col-span-4 flex items-center justify-between">
                  <span className={`rounded-full px-2 py-0.5 text-xs font-semibold ${activityType.status ? 'bg-emerald-50 text-emerald-600 border border-emerald-200' : 'bg-red-50 text-red-600 border border-red-200'}`}>
                    {activityType.status ? 'Active' : 'Inactive'}
                  </span>
                  <div className="relative">
                    <button data-menu-button onClick={() => setMenuOpenId(menuOpenId === (activityType._id || idx) ? null : (activityType._id || idx))} className="rounded-full border border-transparent p-1.5 text-[#8C93AF] transition hover:border-[#E5E8F6] hover:bg-[#F5F7FD] hover:text-[#2D3658]">
                      <MoreVertical className="h-4 w-4" />
                    </button>
                    {menuOpenId === (activityType._id || idx) && (
                      <div data-menu-content className="absolute right-0 top-full mt-1 w-32 rounded-md border border-[#E5E8F6] bg-white shadow-lg z-20">
                        <button onClick={() => startEdit(activityType)} className="flex w-full items-center gap-2 px-3 py-1.5 text-xs text-[#2D3658] hover:bg-[#F6F7FD]"><Pencil className="h-3.5 w-3.5" />Edit</button>
                        <button onClick={() => { setConfirmId(activityType._id); setConfirmOpen(true); setMenuOpenId(null); }} className="flex w-full items-center gap-2 px-3 py-1.5 text-xs text-red-600 hover:bg-red-50"><Trash2 className="h-3.5 w-3.5" />Delete</button>
                      </div>
                    )}
                  </div>
                </div>
            ...
            {!loading && !error && filteredActivityTypes.length === 0 && <div className="px-4 py-4 text-center text-xs text-[#5E6582]">No activity types found</div>}
          </div>
        </div>
      </div>
          ...
          <div className="relative z-50 w-full max-w-sm rounded-xl border border-[#E5E8F6] bg-white p-5 shadow-lg">
            <div className="flex items-start gap-3">
              ...
              <div className="flex-1">
                <div className="text-sm font-semibold text-slate-900">Delete this activity type?</div>
