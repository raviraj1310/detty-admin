...
export default function VendorMaster() {
  ...
  const normalizeVendor = d => ({
    ...
    createdAt: d?.createdAt || d?.updatedAt || (d?.userId && (d.userId.createdAt || d.userId.updatedAt)) || ''
  })
  ...
  const emailRegex = /^(?:[a-zA-Z0-9_'^&\/+{}~-]+(?:\.[a-zA-Z0-9_'^&\/+{}~-]+)*|"(?:[^"]|\\")+")@(?:(?:[a-zA-Z0-9-]+\.)+[a-zA-Z]{2,}|\[(?:[0-9]{1,3}\.){3}[0-9]{1,3}\])$/
  const phoneRegex = /^[0-9]{7,15}$/

  const validate = () => {
    const errs = {}
    if (!formData.name || formData.name.trim().length < 2) errs.name = 'Enter a valid name'
    if (!formData.email || !emailRegex.test(formData.email.trim())) errs.email = 'Enter a valid email'
    if (!formData.password || formData.password.length < 6) errs.password = 'Enter a 6+ char password'
    if (!formData.phoneNumber || !phoneRegex.test(formData.phoneNumber)) errs.phoneNumber = 'Enter a valid phone'
    if (!formData.businessName || formData.businessName.trim().length < 2) errs.businessName = 'Enter a business name'
    return errs
  }
  ...
  const handleSubmit = async e => {
    ...
    try {
      ...
      if (editingId) {
        setVendors(prev => prev.map(v => (v._id === editingId ? { ...v, ...payload } : v)))
        setToast({ open: true, title: 'Vendor updated', description: 'Changes have been saved', variant: 'success' })
      } else {
        ...
        if (dup || (!hasData && res?.success)) {
          setErrors(prev => ({ ...prev, email: msg || 'Email already exists' }))
  ...
  const confirmDelete = id => {
    ...
    setMenuOpenId(null)
    setToast({ open: true, title: 'Vendor deleted', description: 'The vendor has been removed', variant: 'success' })
  }
  ...
  const filtered = vendors.filter(v => {
    ...
    return (
      ...
      String(v.businessName || '').toLowerCase().includes(term) ||
      String(v.phoneNumber || '').toLowerCase().includes(term)
    )
  })
      ...
      <div className='rounded-2xl border border-[#E1E6F7] bg-white p-4 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.55)]'>
        ...
        <div className='overflow-visible rounded-xl border border-[#E5E8F5]'>
          <div className='grid grid-cols-12 gap-4 bg-[#F7F9FD] px-4 py-3'>
            ...
            <div className='col-span-2'><TableHeaderCell onClick={() => toggleSort('business')} active={sortKey === 'business'} order={sortOrder}>Business</TableHeaderCell></div>
            <div className='col-span-1 flex justify-end'><TableHeaderCell align='right'>Action</TableHeaderCell></div>
          </div>
