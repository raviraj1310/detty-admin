      ...
      <div className="min-h-screen bg-gray-50 py-6 px-4 sm:px-5 lg:px-8">
        ...
        <div className="mb-4">
          <h1 className="text-xl sm:text-2xl font-bold text-gray-900">
            {formatHeaderText('Tickets booked')}{headerName ? ` - ${headerName}` : ''}
          </h1>
          <p className="text-xs text-gray-500 mt-1">Dashboard / Tickets booked</p>
        </div>
            ...
            >
              <div className="flex items-center justify-between">
                ...
                <div className="text-right">
                  ...
                  <p className={`text-2xl font-bold ${card.textColor}`}>{card.value} <span className="text-sm font-normal opacity-70">{card.amount}</span></p>
                </div>
              </div>
            </div>
          ))}
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-4">
          <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
            <h2 className="text-base font-semibold text-slate-900">{formatHeaderText('Tickets booked list')}</h2>
            <div className="flex flex-wrap items-center gap-2">
              ...
              <button className="flex h-8 items-center gap-1.5 rounded-lg border border-[#E5E6EF] bg-white px-3 text-xs font-medium text-[#2D3658] transition hover:bg-[#F6F7FD]">
                <svg className="h-3.5 w-3.5 text-[#8B93AF]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
          ...
          <div className="overflow-hidden rounded-xl border border-[#E5E8F5]">
            <div className="grid grid-cols-[1.2fr_1.2fr_1.8fr_1.2fr_1.8fr_0.9fr_1.1fr_0.9fr_1fr_40px] gap-2 bg-[#F7F9FD] px-4 py-3">
              <div><TableHeaderCell>{formatHeaderText('Booked on')}</TableHeaderCell></div>
              <div><TableHeaderCell>{formatHeaderText('User name')}</TableHeaderCell></div>
              <div><TableHeaderCell>{formatHeaderText('Email id')}</TableHeaderCell></div>
              <div><TableHeaderCell>{formatHeaderText('Phone number')}</TableHeaderCell></div>
              <div><TableHeaderCell>{formatHeaderText('Tickets booked')}</TableHeaderCell></div>
              <div><TableHeaderCell>{formatHeaderText('Amount')}</TableHeaderCell></div>
              <div><TableHeaderCell>{formatHeaderText('Arrival date')}</TableHeaderCell></div>
              <div><TableHeaderCell>{formatHeaderText('Payment status')}</TableHeaderCell></div>
              <div><TableHeaderCell>{formatHeaderText('Activity status')}</TableHeaderCell></div>
              <div></div>
            </div>

            <div className="divide-y divide-[#EEF1FA] bg-white">
              {loading && <div className='px-4 py-4 text-xs text-[#5E6582]'>Loading...</div>}
              {error && !loading && <div className='px-4 py-4 text-xs text-red-600'>{error}</div>}
              {!loading && !error && rows.map((booking) => (
                ...
                >
                  <div className="self-center text-xs text-[#5E6582]">{booking.bookedOn}</div>
                  <div className="self-center text-xs font-medium text-slate-900">{booking.userName}</div>
                  <div className="self-center text-xs text-[#5E6582] truncate">{booking.email}</div>
                  <div className="self-center text-xs text-[#5E6582]">{booking.phoneNumber}</div>
                  <div className="self-center text-[10px] text-[#5E6582] leading-relaxed whitespace-pre-line">{booking.ticketsBooked}</div>
                  <div className="self-center text-xs font-semibold text-slate-900">{booking.amount}</div>
                  <div className="self-center text-xs font-semibold text-slate-900">{booking.arrivalDate}</div>
                  <div className="flex items-center self-center">
                    <span className={`inline-flex items-center justify-center rounded-full px-2 py-0.5 text-[10px] font-semibold ${booking.statusClass}`}>
                      {booking.paymentStatus}
                    </span>
                  </div>
                  <div className="flex items-center self-center">
                    <span className={`inline-flex items-center justify-center rounded-full px-2 py-0.5 text-[10px] font-semibold ${booking.activityStatusClass}`}>
                      {booking.status}
                    </span>
                  </div>
                        ...
                        {(() => {
                          ...
                          return (
                            <button
                              ...
                              className={`flex w-full items-center gap-2 px-3 py-2 text-xs ${isDownloading ? 'text-[#8C93AF] cursor-not-allowed opacity-70' : 'text-[#2D3658] hover:bg-[#F6F7FD]'}`}
                            >
                              {isDownloading ? (
              ...
              {!loading && !error && rows.length === 0 && (
                <div className='px-4 py-4 text-xs text-[#5E6582]'>No bookings found</div>
              )}
            </div>
          </div>
        </div>
      ...
      {ticketOpen && selectedBooking && (
        <div className='fixed inset-0 z-40 flex items-center justify-center'>
          ...
          <div className='relative z-50 w-full max-w-lg rounded-xl border border-[#E5E8F6] bg-white p-5 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.55)]'>
            <div className='text-base font-semibold text-slate-900 mb-3'>Ticket</div>
            ...
            <div className='mt-5 flex justify-end gap-3'>
              <button onClick={() => { setTicketOpen(false); setSelectedBooking(null) }} className='rounded-lg border border-[#E5E6EF] bg-white px-4 py-2 text-xs font-medium text-[#1A1F3F] shadow-sm transition hover:bg-[#F9FAFD]'>Close</button>
            </div>
          </div>
        </div>
      )}
          ...
          <div className='space-y-4'>
            <div className='rounded-lg bg-[#F8F9FC] p-3'>
              <div className='grid grid-cols-2 gap-3 text-xs'>
                ...
                <div className='text-right font-semibold text-slate-900'>{(() => { const c = selectedBooking.buyer || (Array.isArray(selectedBooking.tickets) && selectedBooking.tickets[0]?.attendees?.[0]) || {}; return c?.fullName || '-' })()}</div>
                <div className='text-[#5E6582]'>Email address</div>
                <div className='text-right font-semibold text-slate-900'>{(() => { const c = selectedBooking.buyer || (Array.isArray(selectedBooking.tickets) && selectedBooking.tickets[0]?.attendees?.[0]) || {}; return c?.email || '-' })()}</div>
                <div className='text-[#5E6582]'>Phone</div>
                <div className='text-right font-semibold text-slate-900'>{(() => { const c = selectedBooking.buyer || (Array.isArray(selectedBooking.tickets) && selectedBooking.tickets[0]?.attendees?.[0]) || {}; return c?.phone || '-' })()}</div>
              </div>
            </div>
            {Array.isArray(selectedBooking.tickets) && selectedBooking.tickets.some(t => Array.isArray(t.attendees) && t.attendees.length > 0) ? (
              <div className='rounded-lg border border-[#E5E8F6] bg-white p-3'>
                <div className='text-xs font-semibold text-slate-900 mb-2'>Attendees</div>
                <div className='space-y-2'>
                  {selectedBooking.tickets.flatMap(t => (Array.isArray(t.attendees) ? t.attendees.map(a => ({ ...a })) : [])).map((a, i) => (
                    <div key={`att-${i}`} className='grid grid-cols-2 gap-3 text-xs'>
            ...
            <div className='mt-3 pt-3 border-t border-[#EEF1FA]'>
              <div className='flex items-center justify-between'>
                ...
                <span className='flex items-center gap-2 text-sm font-bold text-slate-900'>{(() => { const amt = selectedBooking?.pricing?.total; return typeof amt === 'number' ? `₦${amt.toLocaleString()}` : amt || '-' })()}</span>
              </div>
            </div>
          </div>
        </Modal>
