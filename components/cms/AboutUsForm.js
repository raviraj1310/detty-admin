...
export default function AboutUsForm() {
  ...
  const [formData, setFormData] = useState({
    ...
    bannerSubTitle: 'Explore top-rated stays across Lagos, from luxury suites to cozy Airbnbs. Find trusted accommodation in the heart of the city.',
    bannerImage: 'image.jpg',
    bannerCTAText: 'Explore Detty Fest',
    bannerCTALink: 'www.link.com',
    promoTitle: 'About 200+ Users are  purchasing ticket for the "Walk the canopy bridge at Lekki Conservation Centre" — Tickets from ₦6,000.',
    ...
    aboutDescription: 'DettyFusion is the ultimate December experience, curating a lineup of events that showcase the very best of Lagos. From live concerts and beach raves to art showcases, food fairs, and family-friendly experience. Our mission is simple: make Lagos the world\'s most exciting December destination.',
    ...
    whyLagosDescription: 'Lagos is more than a city — it\'s a heartbeat. Known for its unmatched nightlife, bustling markets, rich culture, and warm people, Lagos transforms every December into a global hotspot.',
    ...
    whoWeAreDescription: 'Whether you\'re a Lagosian, a Nigerian returning home for the holidays, or an international traveler discovering Africa, DettyFusion is built for you.',
    whoWeAreImage: 'image.jpg',
    whoWeAreCTAText: 'Browse All Bundles',
    whoWeAreCTALink: 'www.link.com',
    promo2Title: 'About 20+ Users are  purchasing ticket for the "Fela & the Kalakuta Queens" — Tickets from ₦5,000.',
  ...
  const whyLagosDescriptionEditor = useEditor({
    ...
    immediatelyRender: false,
    onUpdate: ({ editor }) => {
      setFormData(prev => ({ ...prev, whyLagosDescription: editor.getHTML() }));
    }
  });
  ...
  const whoWeAreDescriptionEditor = useEditor({
    ...
    immediatelyRender: false,
    onUpdate: ({ editor }) => {
      setFormData(prev => ({ ...prev, whoWeAreDescription: editor.getHTML() }));
    }
  });
  ...
  useEffect(() => {
    const load = async () => {
      try {
        ...
        const cleanLink = v => String(v || '').replace(/[`]/g, '').trim();
        const mapped = {
          bannerTitle: d?.bannerSection?.title ?? d?.bannerSectionTitle ?? formData.bannerTitle,
          bannerSubTitle: d?.bannerSection?.subTitle ?? d?.bannerSectionSubTitle ?? formData.bannerSubTitle,
          bannerImage: d?.bannerSectionBannerImage ?? d?.bannerSection?.bannerImage ?? formData.bannerImage,
          bannerCTAText: d?.bannerSection?.CTAText ?? d?.bannerSectionCTAText ?? formData.bannerCTAText,
          bannerCTALink: cleanLink(d?.bannerSection?.CTALink ?? d?.bannerSectionCTALink ?? formData.bannerCTALink),
          promoTitle: d?.promoSection?.title ?? d?.promoSectionTitle ?? formData.promoTitle,
          promoImage: d?.promoSectionPromoImage ?? d?.promoSection?.promoImage ?? formData.promoImage,
          promoCTAText: d?.promoSection?.CTAText ?? d?.promoSectionCTAText ?? formData.promoCTAText,
          promoCTALink: cleanLink(d?.promoSection?.CTALink ?? d?.promoSectionCTALink ?? formData.promoCTALink),
          aboutSectionTitle: d?.aboutSection?.sectionTitle ?? d?.aboutSectionSectionTitle ?? formData.aboutSectionTitle,
          aboutTitle: d?.aboutSection?.title ?? d?.aboutSectionTitle ?? formData.aboutTitle,
          aboutDescription: d?.aboutSection?.description ?? d?.aboutSectionDescription ?? formData.aboutDescription,
          aboutImage: d?.aboutSectionImage ?? formData.aboutImage,
          whyLagosSectionTitle: d?.whyLagosSection?.sectionTitle ?? d?.whyLagosSectionSectionTitle ?? formData.whyLagosSectionTitle,
          whyLagosTitle: d?.whyLagosSection?.title ?? d?.whyLagosSectionTitle ?? formData.whyLagosTitle,
          whyLagosDescription: d?.whyLagosSection?.description ?? d?.whyLagosSectionDescription ?? formData.whyLagosDescription,
          whyLagosImage1: d?.whyLagosSectionImage1 ?? d?.whyLagosSection?.image1 ?? formData.whyLagosImage1,
          whyLagosImage2: d?.whyLagosSectionImage2 ?? d?.whyLagosSection?.image2 ?? formData.whyLagosImage2,
          whyLagosImage3: d?.whyLagosSectionImage3 ?? d?.whyLagosSection?.image3 ?? formData.whyLagosImage3,
          whyLagosImage4: d?.whyLagosSectionImage4 ?? d?.whyLagosSection?.image4 ?? formData.whyLagosImage4,
          whoWeAreSectionTitle: d?.whoWeAre?.sectionTitle ?? d?.whoWeAreSectionTitle ?? formData.whoWeAreSectionTitle,
          whoWeAreTitle: d?.whoWeAre?.title ?? d?.whoWeAreTitle ?? formData.whoWeAreTitle,
          whoWeAreDescription: d?.whoWeAre?.description ?? d?.whoWeAreDescription ?? formData.whoWeAreDescription,
          whoWeAreImage: d?.whoWeAreImage ?? formData.whoWeAreImage,
          promo2Title: d?.promoSection2?.title ?? d?.promoSection2Title ?? formData.promo2Title,
          promo2Image: d?.promoSection2UploadImage ?? d?.promoSection2?.uploadImage ?? formData.promo2Image,
          promo2CTAText: d?.promoSection2?.CTAText ?? d?.promoSection2CTAText ?? formData.promo2CTAText,
          promo2CTALink: cleanLink(d?.promoSection2?.CTALink ?? d?.promoSection2CTALink ?? formData.promo2CTALink),
          tourGuideSectionTitle: d?.tourGuidSection?.sectionTitle ?? d?.tourGuidSectionSectionTitle ?? formData.tourGuideSectionTitle,
          tourGuideTitle: d?.tourGuidSection?.title ?? d?.tourGuidSectionTitle ?? formData.tourGuideTitle,
          tourGuideImage: d?.tourGuidSectionImage ?? d?.tourGuidSection?.uploadImage ?? formData.tourGuideImage,
          tourGuideCTAText: d?.tourGuidSection?.CTAText ?? d?.tourGuidSectionCTAText ?? formData.tourGuideCTAText,
          tourGuideFile: d?.tourGuidSectionFile ?? d?.tourGuidSection?.file ?? formData.tourGuideFile,
        };
        ...
        if (whyLagosDescriptionEditor && mapped.whyLagosDescription) {
          whyLagosDescriptionEditor.commands.setContent(mapped.whyLagosDescription);
        }
        if (whoWeAreDescriptionEditor && mapped.whoWeAreDescription) {
          whoWeAreDescriptionEditor.commands.setContent(mapped.whoWeAreDescription);
        }
        ...
        const imgFields = ['bannerImage','promoImage','aboutImage','whyLagosImage1','whyLagosImage2','whyLagosImage3','whyLagosImage4','whoWeAreImage','promo2Image','tourGuideImage'];
        const nextPreviews = {};
        imgFields.forEach(k => {
          const v = mapped[k];
        ...
        const msg = err?.response?.data?.message || err?.message || 'Failed to fetch About Us';
        setToast({ open: true, title: 'Error', description: msg, variant: 'error' });
      }
    };
    load();
  }, [aboutDescriptionEditor, whyLagosDescriptionEditor, whoWeAreDescriptionEditor, promoTitleEditor, promo2TitleEditor]);
  ...
  const sections = [
    {
      ...
      fields: [
        ...
        { name: 'bannerSubTitle', label: 'Sub Title', type: 'textarea', required: true, rows: 3 },
        { name: 'bannerImage', label: 'Upload Image', type: 'file', required: true },
        { name: 'bannerCTAText', label: 'CTA Text', type: 'text', required: true },
        { name: 'bannerCTALink', label: 'CTA Link', type: 'text', required: true }
      ]
    },
    {
      id: 'promo',
      title: 'Promo Section',
      fields: [
        { name: 'promoTitle', label: 'Title', type: 'richtext', required: true, rows: 2 },
        { name: 'promoImage', label: 'Upload Image', type: 'file', required: true },
        { name: 'promoCTAText', label: 'CTA Text', type: 'text', required: true },
        { name: 'promoCTALink', label: 'CTA Link', type: 'text', required: true }
      ]
    },
    {
      id: 'about',
      title: 'About Section',
      fields: [
        { name: 'aboutSectionTitle', label: 'Section Title', type: 'text', required: true },
        { name: 'aboutTitle', label: 'Title', type: 'text', required: true },
        { name: 'aboutDescription', label: 'Description', type: 'richtext', required: true, rows: 3 },
        { name: 'aboutImage', label: 'Upload Image', type: 'file', required: true },
        { name: 'aboutCTAText', label: 'CTA Text', type: 'text', required: true },
        { name: 'aboutCTALink', label: 'CTA Link', type: 'text', required: true }
      ]
    },
    {
      id: 'whyLagos',
      ...
      fields: [
        { name: 'whyLagosSectionTitle', label: 'Section Title', type: 'text', required: true },
        { name: 'whyLagosTitle', label: 'Title', type: 'text', required: true },
        { name: 'whyLagosDescription', label: 'Description', type: 'richtext', required: true, rows: 3 },
        { name: 'whyLagosImage1', label: 'Upload Image 1', type: 'file', required: true },
        { name: 'whyLagosImage2', label: 'Upload Image 2', type: 'file', required: true },
        { name: 'whyLagosImage3', label: 'Upload Image 3', type: 'file', required: true },
        { name: 'whyLagosImage4', label: 'Upload Image 4', type: 'file', required: true }
      ]
    },
    {
      id: 'whoWeAre',
      ...
      fields: [
        { name: 'whoWeAreSectionTitle', label: 'Section Title', type: 'text', required: true },
        { name: 'whoWeAreTitle', label: 'Title', type: 'text', required: true },
        { name: 'whoWeAreDescription', label: 'Description', type: 'richtext', required: true, rows: 3 },
        { name: 'whoWeAreImage', label: 'Upload Image', type: 'file', required: true },
        { name: 'whoWeAreCTAText', label: 'CTA Text', type: 'text', required: true },
        { name: 'whoWeAreCTALink', label: 'CTA Link', type: 'text', required: true }
      ]
    },
    {
      id: 'promo2',
      title: 'Promo Section',
      fields: [
        { name: 'promo2Title', label: 'Title', type: 'richtext', required: true, rows: 2 },
        { name: 'promo2Image', label: 'Upload Image', type: 'file', required: true },
        { name: 'promo2CTAText', label: 'CTA Text', type: 'text', required: true },
        { name: 'promo2CTALink', label: 'CTA Link', type: 'text', required: true }
      ]
    },
    {
      id: 'tourGuide',
      ...
      fields: [
        { name: 'tourGuideSectionTitle', label: 'Section Title', type: 'text', required: true },
        { name: 'tourGuideTitle', label: 'Title', type: 'text', required: true },
        { name: 'tourGuideImage', label: 'Upload Image', type: 'file', required: true },
        { name: 'tourGuideCTAText', label: 'CTA Text', type: 'text', required: true },
        { name: 'tourGuideFile', label: 'Upload File', type: 'file', required: true }
      ]
    }
  ];
  ...
  const handleCropped = ({ file }) => {
    ...
    handleChange(cropField, file.name);
    setImagePreviewUrls(prev => {
      const next = { ...prev };
      if (next[cropField]) {
        try { URL.revokeObjectURL(next[cropField]); } catch {}
      }
      try { next[cropField] = URL.createObjectURL(file); } catch { next[cropField] = ''; }
      return next;
    });
  };
  ...
  const validateForm = () => {
    const e = {};
    const req = [
      'bannerTitle','bannerSubTitle','bannerImage','bannerCTAText','bannerCTALink',
      'promoTitle','promoImage','promoCTAText','promoCTALink',
      'aboutSectionTitle','aboutTitle','aboutDescription',
      'whyLagosSectionTitle','whyLagosTitle','whyLagosDescription','whyLagosImage1','whyLagosImage2','whyLagosImage3','whyLagosImage4',
      'whoWeAreSectionTitle','whoWeAreTitle','whoWeAreDescription',
      'promo2Title','promo2Image','promo2CTAText','promo2CTALink',
      'tourGuideSectionTitle','tourGuideTitle','tourGuideImage','tourGuideCTAText','tourGuideFile'
    ];
    req.forEach(k => { if (!String(formData[k] || '').trim()) e[k] = 'Required'; });
    const urlFields = ['bannerCTALink','promoCTALink','promo2CTALink'];
    urlFields.forEach(k => { const v = String(formData[k] || '').trim(); if (v && !/^https?:\/\//i.test(v)) e[k] = 'Enter a valid URL'; });
    return e;
  };
  ...
  const toPayload = () => ({
    ...
    promoSection: {
      title: promoTitleEditor?.getHTML() || String(formData.promoTitle || '').trim(),
    ...
    promoSectionTitle: promoTitleEditor?.getHTML() || String(formData.promoTitle || '').trim(),
    ...
    aboutSection: {
      ...
      description: aboutDescriptionEditor?.getHTML() || String(formData.aboutDescription || '').trim(),
    ...
    aboutSectionDescription: aboutDescriptionEditor?.getHTML() || String(formData.aboutDescription || '').trim(),
    ...
    whyLagosSection: {
      ...
      description: whyLagosDescriptionEditor?.getHTML() || String(formData.whyLagosDescription || '').trim(),
    ...
    whyLagosSectionSectionTitle: String(formData.whyLagosSectionTitle || '').trim(),
    whyLagosSectionTitle: String(formData.whyLagosTitle || '').trim(),
    whyLagosSectionDescription: whyLagosDescriptionEditor?.getHTML() || String(formData.whyLagosDescription || '').trim(),
    ...
    whoWeAre: {
      ...
      description: whoWeAreDescriptionEditor?.getHTML() || String(formData.whoWeAreDescription || '').trim(),
    ...
    whoWeAreDescription: whoWeAreDescriptionEditor?.getHTML() || String(formData.whoWeAreDescription || '').trim(),
    ...
    promoSection2: {
      title: promo2TitleEditor?.getHTML() || String(formData.promo2Title || '').trim(),
    ...
    promoSection2Title: promo2TitleEditor?.getHTML() || String(formData.promo2Title || '').trim(),
    ...
    tourGuidSectionSectionTitle: String(formData.tourGuideSectionTitle || '').trim(),
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      const res = await createAndUpdateAbout(payload);
      setToast({ open: true, title: 'Saved', description: 'About Us updated successfully', variant: 'success' });
    } catch (err) {
  ...
  const renderRichTextEditor = (field, rows = 2) => {
    ...
    return (
      <div className={`tiptap-editor-wrapper border border-gray-300 rounded-lg overflow-hidden ${isFullscreen[field] ? 'fixed inset-0 z-50 bg-white' : ''}`}>
  ...
  return (
    <div className="p-4 sm:p-6 lg:p-8 bg-white">
      ...
      <Toast
        ...
        position='top-right'
      />
      <ImageCropper open={cropOpen} file={cropFile} onClose={handleCropClose} onCropped={handleCropped} />
    </div>
  );
}
