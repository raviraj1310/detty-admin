...
import {getTermsOfUse, createUpdateTermsPrivacy} from '@/services/cms/terms-privacy.service';
...
export default function TermsOfUseForm() {
  ...
  useEffect(() => {
    const fetchTerms = async () => {
      ...
      try {
        ...
        setToast({ open: true, title: 'Error', description: 'Failed to fetch Terms of Use', variant: 'error' });
      } finally {
        setLoading(false);
      }
    };
  ...
  const handleSave = async () => {
    try {
      ...
      setToast({ open: true, title: 'Saved', description: 'Terms of Use updated', variant: 'success' });
    } catch (e) {
      const msg = e?.response?.data?.message || e?.message || 'Failed to save Terms of Use';
      ...
      <div className="bg-gray-200 p-3 sm:p-4 lg:p-6 rounded-xl">
        ...
        <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-4 sm:p-6">
          ...
          <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-3 sm:gap-0 mb-4 sm:mb-4">
            <h2 className="text-lg sm:text-xl font-semibold text-gray-900">Terms of Use Details</h2>
            <button
              onClick={handleSave}
              disabled={saving}
          ...
          <div>
            ...
            />
            <label className="block text-sm font-medium text-gray-700 mb-2 mt-4">Terms of Use<span className="text-red-500">*</span></label>
            <TiptapEditor
