...
export default function PermissionMaster() {
  ...
  const fetchPermissions = async () => {
    ...
    setError("");
    try {
      const res = await getAllPermissions(1, 1000); // Fetch all (limit 1000)
  ...
  const filteredPermissions = useMemo(() => {
    ...
    return base.filter((a) => {
      ...
      return !term
        ? true
        : name.includes(term) || desc.includes(term) || addedStr.includes(term);
    });
              ...
              sortedPermissions.map((item, idx) => (
                ...
                >
                  ...
                  <div className="col-span-4 flex items-center justify-between gap-4">
                     <div className="truncate text-xs text-[#5E6582] max-w-[150px]" title={item.description}>
                        {item.description || "-"}
                     </div>
