    ...
    <div className="space-y-7 py-12 px-12">
      <div className="flex flex-col gap-4 md:flex-row md:items-start md:justify-between">
        <div className="flex flex-col gap-2">
          <h1 className="text-2xl font-semibold text-slate-900">Email Subscriptions</h1>
          <p className="text-sm text-[#99A1BC]">Dashboard / Email Subscriptions</p>
        </div>
      </div>
          ...
          <div key={card.id} className={`${card.bg} rounded-2xl p-6 text-white relative overflow-hidden`}>
            <div className="flex items-center justify-between">
              ...
              <div className="text-right">
                ...
                <p className="text-4xl font-bold text-white">{String(card.id === 'total' ? metrics.total : card.id === 'new' ? metrics.new : metrics.unsub)}</p>
              </div>
            </div>
          </div>
        ))}
      ...
      <div className="rounded-[30px] border border-[#E1E6F7] bg-white p-6 shadow-[0_24px_60px_-40px_rgba(15,23,42,0.55)]">
        <div className="mb-6 flex flex-wrap items-center justify-between gap-4">
          <h2 className="text-lg font-semibold text-slate-900">Subscriptions List</h2>
          <div className="flex flex-wrap items-center gap-3">
            ...
            <button className="flex h-10 items-center gap-2 rounded-xl border border-[#E5E6EF] bg-white px-4 text-sm font-medium text-[#2D3658] transition hover:bg-[#F6F7FD]" onClick={downloadCsv}>
        ...
        <div className="overflow-visible rounded-2xl border border-[#E5E8F5]">
          <div className="grid grid-cols-[1.5fr_2fr_1.2fr] gap-3 bg-[#F7F9FD] px-6 py-4">
          ...
          <div className="divide-y divide-[#EEF1FA] bg-white">
            ...
            {!loading && !error && sorted.map((s, idx) => (
              <div key={s.id || idx} className="grid grid-cols-[1.5fr_2fr_1.2fr] gap-3 px-6 py-5 hover:bg-[#F9FAFD]">
                ...
                <div className="self-center text-sm text-[#5E6582] truncate">{s.email || '-'}</div>
                <div className="self-center">
                  <span className={`inline-flex items-center justify-center rounded-full px-3 py-1 text-xs font-semibold ${String(s.status).toLowerCase() === 'unsubscribed' ? 'bg-red-50 text-red-600 border border-red-200' : 'bg-emerald-50 text-emerald-600 border border-emerald-200'}`}>
                    {s.status || 'Subscribed'}
                  </span>
                </div>
              </div>
